...
def chat(cfg: Config, messages: list[Message], *, module: str | None = None,
    ...
    # ДОП. ЗАДАНИЕ ПОЛЬЗОВАТЕЛЯ для этого модуля (ТЗ 27.07: «окно в каждом
    # разделе для доп. задания ИИ — переформатируй так-то, добавь то-то»).
    # Подмешивается в системное сообщение, поэтому работает во ВСЕХ модулях.
    if module:
        extra = str(cfg.get(f"ai.modules.{module}.instruction", "") or "").strip()
        if extra:
            messages = list(messages)
            sys_i = next((i for i, m in enumerate(messages)
                          if m.get("role") == "system"), None)
            add = f"\n\nДОПОЛНИТЕЛЬНОЕ УКАЗАНИЕ ПОЛЬЗОВАТЕЛЯ (соблюдай строго):\n{extra}"
            if sys_i is None:
                messages.insert(0, {"role": "system", "content": add.strip()})
            else:
                messages[sys_i] = {**messages[sys_i],
                                   "content": messages[sys_i].get("content", "") + add}
