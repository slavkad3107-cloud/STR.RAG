...
def sidebar() -> tuple[str, str]:
    ...
    if mode == "Создать новый" or not projects:
        ...
        picked = st.sidebar.selectbox("Проект (объект)", projects, index=idx,
                                      key="sb_project_pick")
        project = st.session_state.get("project") or picked
        # ПОДТВЕРЖДЕНИЕ СМЕНЫ ОБЪЕКТА (ТЗ 27.07): переключение меняет данные во
        # ВСЕХ вкладках, поэтому спрашиваем явно — иначе легко начать работать
        # не с тем объектом и не заметить.
        if picked != project:
            st.sidebar.warning(f"Перейти к объекту «{picked}»? Данные во всех "
                               f"вкладках сменятся.")
            cy, cn = st.sidebar.columns(2)
            if cy.button("Да, перейти", key="sb_prj_yes", width='stretch'):
                st.session_state.project = picked
                st.session_state.pop("object_type", None)
                st.rerun()
            if cn.button("Отмена", key="sb_prj_no", width='stretch'):
                st.session_state["sb_project_pick"] = project
                st.rerun()

    # ТИП ОБЪЕКТА (ПП-87) ЗАФИКСИРОВАН за проектом (ТЗ 27.07: «надо делать
    # выбранным сразу/зафиксированным, а то путаница происходит постоянно»).
    # Меняется только осознанно — с полной переиндексацией в М2.
    cfg = _cfg()
    object_type = ""
    if project:
        try:
            from pmoos.index.indexer import read_state as _rs_ot
            object_type = _rs_ot(project).get("object_type") or ""
        except Exception:  # noqa: BLE001
            object_type = ""
        if not object_type:  # ещё не индексировали — определяем по файлам
            try:
                from pmoos.ingest.inventory import load_inventory as _li_ot
                object_type = (_li_ot(project) or {}).get("object_type") or ""
            except Exception:  # noqa: BLE001
                object_type = ""
    object_type = object_type or cfg.get("object_type", "площадной")
    st.session_state.object_type = object_type
    st.sidebar.caption(f"Тип объекта (ПП-87): **{object_type}** — зафиксирован "
                       f"за проектом (смена только через «♻ Переиндексировать» в М2)")
...
def tab_m1(project: str, object_type: str) -> None:
    ...
    st.caption("Файлы проекта НЕ храним: после успешной индексации исходники удаляются "
               "АВТОМАТИЧЕСКИ — в базе остаются чанки, карта разделов и производные "
               "данные (этого достаточно для ответов). Вернуть хранение: "
               "storage.keep_sources: true в config.yaml.")
    ...
    # Организация-заказчик/проектировщик — видна в шапке на всех вкладках
    _inv_org = (_inv0 or {}).get("organization", "") if _inv0 else ""
    _org_new = st.text_input("🏢 Организация (заказчик / проектировщик)",
                             value=_inv_org, key=f"org_{project}",
                             placeholder="ООО «…» — видно в шапке над вкладками")
    if _org_new.strip() != str(_inv_org or "").strip():
        try:
            import json as _js
            _p_inv = project_paths(project)["inventory"]
            _d_inv = _li(project) or {}
            _d_inv["organization"] = _org_new.strip()
            _p_inv.parent.mkdir(parents=True, exist_ok=True)
            _p_inv.write_text(_js.dumps(_d_inv, ensure_ascii=False, indent=2),
                              encoding="utf-8")
            st.rerun()
...
def main() -> None:
    ...
    # ШАПКА: чётко видно, по какой организации и какому объекту работаем
    # (ТЗ 27.07). Организация задаётся в М1 и хранится в карточке проекта.
    _org = ""
    try:
        from pmoos.ingest.inventory import load_inventory as _li_h
        _org = ((_li_h(project) or {}).get("organization") or "").strip()
    except Exception:  # noqa: BLE001
        _org = ""
