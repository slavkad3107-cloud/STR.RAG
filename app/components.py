...
def module_ai_selector(cfg, module: str, *, title: str | None = None) -> None:
    ...
    with st.expander(title, expanded=not (cfg.has_key(cur) or cur == "ollama")):
        ...
        # ── ДОП. ЗАДАНИЕ ИИ для этого модуля (ТЗ 27.07) ──
        _ikey = f"ai.modules.{module}.instruction"
        _cur_instr = str(cfg.get(_ikey, "") or "")
        _new_instr = st.text_area(
            "📝 Доп. задание ИИ для этого модуля (необязательно)",
            value=_cur_instr, key=f"instr_{module}", height=90,
            placeholder="Например: «ответы формулируй от лица генпроектировщика», "
                        "«всегда указывай пункт норматива», «таблицы выводи в "
                        "формате как в томе 6.1»",
            help="Текст добавляется к заданию ИИ КАЖДЫЙ раз в этом модуле. "
                 "Сохраняется в настройках проекта.")
        if _new_instr.strip() != _cur_instr.strip():
            cfg.set(_ikey, _new_instr.strip())
            cfg.save()
            st.caption("✅ Задание сохранено — применится при следующем запуске модуля.")
    ...
    with st.expander(title, expanded=not health):
        ...
        if health:
            ...
            for p, r in sorted(health.items(),
                ...
                from pmoos.core.health import TIERS as _T
                rows.append({"Провайдер": PROVIDER_LABEL.get(p, p),
                             "Тип": TIER_RU.get(_T.get(p, r.get("tier", 9)), "?"),
                             "Состояние": state,
...
def indexing_panel(project: str, object_type: str) -> None:
    ...
    if st.session_state.get("idx_reindex_arm"):
        ...
        # ЕДИНСТВЕННОЕ место смены типа объекта (ТЗ 27.07: тип зафиксирован,
        # менять только осознанно — вместе с полной переиндексацией)
        _types = ["площадной", "линейный"]
        object_type = rc2.selectbox(
            "Тип объекта (ПП-87) для новой индексации", _types,
            index=_types.index(object_type) if object_type in _types else 0,
            key="idx_reindex_ot",
            help="У линейных объектов есть ТКР и ППО (Разделы 3 и 2). "
                 "После переиндексации тип фиксируется за проектом.")
        yy, nn = rc2.columns(2)
