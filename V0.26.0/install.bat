...
echo ============================================================
echo   STROYPROEKT - install (Windows)
echo ============================================================
...
echo.
echo [3/5] Installing PyTorch (CPU build - works on any computer) ...
echo     Small download, no NVIDIA drivers needed. GPU users: see README, section GPU.
pip install torch==2.6.0 --index-url https://download.pytorch.org/whl/cpu --timeout 180 --retries 10
if not errorlevel 1 goto TORCHOK
echo WARNING: CPU torch index failed. Trying default PyPI ...
