...
INDICATORS: list[dict] = [
    ...
    {"key": "length_route", "label": "Протяжённость трассы", "unit": "км",
     ...
     # ОПОЧКА 18.09: «10 км» приходило из Водного кодекса («для ручья
     # протяжённостью менее 10 км»), «30/610 км» — длины рек из изысканий
     "skip": r"рек|руч|водоток|русл|исток|усть|водоохранн|лэп|кабел"},
    {"key": "duration", "label": "Продолжительность строительства", "unit": "мес.",
     ...
     "syn": [r"продолжительност\w* строительства", r"срок строительства",
             r"общая продолжительност\w* (?:производства )?работ"],
     # «продолжительность строительства ОДНОГО локального очистного сооружения
     # 1,5 месяца» — срок части объекта, а не стройки
     "skip": r"одного|одной|локальн|очистн|лос|подготовительн|норм\w* продолжительн|снип"},
    ...
    {"key": "szz", "label": "Размер санитарно-защитной зоны", "unit": "м",
     ...
     # «СЗЗ 1000 м» — радиус запроса о скотомогильниках в справке ветеринарии
     "skip": r"скотомогильн|биотермическ|захоронен|сибиреязв|кладбищ|в каждую сторону|таких объектов"},
...
def _norm_num(s: str) -> str:
    v = re.sub(r"[   ]", "", (s or "")).replace(",", ".").rstrip(".")
    # «10.0» и «10» — одно значение (иначе ложное «расхождение» 10.0 ↔ 10 мес.)
...
def _scan_text(text: str, ind: dict) -> list[str]:
    ...
    for syn in ind["syn"]:
        for m in re.finditer(syn, low):
            ...
            if hit:
                # ЧУЖОЙ КОНТЕКСТ: показатель относится не к объекту (река, одно
                # сооружение, справка о скотомогильниках) либо число — порог
                # нормы («менее 10 км», «до 50 м»), а не значение
                before = low[max(0, m.start() - 90): m.start()]
                before = re.split(r"[.;:]\s", before)[-1]      # только своё предложение
                ctx = before + low[m.start(): m.end()] + window[: hit.start()]
                if ind.get("skip") and re.search(ind["skip"], ctx):
                    continue
                if re.search(r"(?:менее|более|свыше|не более|не менее|до|от)\s*$", window[: hit.start()]):
                    continue
...
def extract_from_index(project: str, cfg=None, *, progress=None,
    ...
    for key, rec in inds.items():
        ...
        if rec.get("source") in ("manual", "chosen"):
            ...
            elif rec.get("source") == "auto":
                # прежнее авто-значение больше ничем не подтверждено (кандидат
                # отсеян как чужой контекст) — не держим его в реестре
                rec["value"] = ""
                rec["provenance"] = {}
                rec["source"] = None
