...
CHAPTERS: dict[str, list[str]] = {
    # структура ООС — по ЭТАЛОНУ пользователя (принятый «Раздел ПД №8
    # АК-01-25-ООС», 337 стр., OneDrive\Формы\Разработка\ООС)
    "OOS": [
        "Введение",
        "Характеристика природных условий района (местоположение, климат, "
        "геология, почвы, современное состояние ОС, растительный и животный "
        "мир, водные ресурсы, ООПТ и зоны с особыми условиями)",
        "Проектные решения",
        "Охрана земельных ресурсов (оценка воздействия и мероприятия)",
        "Охрана атмосферного воздуха (оценка на периоды строительства и "
        "эксплуатации, мероприятия, предложения по нормативам ПДВ)",
        ...
        "транспортировке и размещению отходов (строительство и эксплуатация)",
    ...
    # структура ИЭИ — по ЭТАЛОНУ пользователя (принятый «Отчет ИЭИ Архангельск»,
    # OneDrive\Формы\Разработка\ИЭИ), а не по общим представлениям
    "IEI": [
        "Введение (цели и задачи, сведения об объекте, состав и объёмы работ)",
        "Анализ фондовых данных и статистических материалов по оценке состояния окружающей среды",
        "Оценка современного экологического состояния участка изысканий, методика работ",
        "Результаты обследования территории участка изысканий",
        "Социально-экономические и медико-демографические условия района изысканий",
        "Предварительный прогноз неблагоприятного воздействия на окружающую среду",
        "Рекомендации и предложения по предотвращению и снижению воздействия",
        "Предложения по организации экологического мониторинга",
    ],
    # структура ОЦЕНКИ — по ЭТАЛОНУ пользователя («Оценка п. Пересыпь»,
    # ФГБУ «Главрыбвод»): это оценка воздействия на ВОДНЫЕ БИОРЕСУРСЫ
    "OCENKA": [
        "Краткая характеристика планируемой деятельности",
        "Материал и методика",
        "Гидробиологическая характеристика водного объекта",
        "Оценка воздействия и определение размера вреда водным биологическим ресурсам",
        "Мероприятия по предотвращению негативного воздействия на водные биоресурсы",
        "Список использованных источников",
    ],
}

# показатель реестра → индекс главы ООС в CHAPTERS["OOS"], 0-based:
# 2=Проектные решения, 3=Земельные, 4=Воздух, 5=Вода, 7=Акустика, 8=Отходы.
# При правке структуры эталона проверьте индексы — их стережёт тест
# test_section_draft (сверяет emission_year с позицией главы воздуха).
_IND_CHAPTER_OOS = {
    "length_route": 2, "duration": 2, "workers": 2,
    "area_build": 3, "area_plot": 3,
    "emission_year": 4, "emission_sec": 4, "sources_count": 4, "szz": 4,
    "water_use": 5, "water_drain": 5,
    "noise": 7,
    "waste_year": 8,
}
