...
def test_registry_skips_foreign_context():
    # ОПОЧКА 18.09 (тестировщик №3): в генерацию уходили «10 км» (длина ручья из
    # Водного кодекса), «1,5 мес.» (срок одного ЛОС), «СЗЗ 1000 м» (радиус
    # запроса о скотомогильниках) — чужой контекст и пороги норм отсекаются
    from pmoos.data import registry as R
    B = R._BY_KEY
    t1 = ("Продолжительность строительства одного локального очистного сооружения составляет 1,5 месяца. "
          "Общая продолжительность производства работ с параллельным переустройством инженерных сетей "
          "составляет 10,0мес., в том числе подготовительный период 1,0мес. Продолжительность строительства 10,0 мес ")
    assert set(R._scan_text(t1, B["duration"])) == {"10"}
    t2 = ("для ручья без названия протяженность менее 10 км устанавливается водоохранная зона. "
          "Протяженность реконструируемой 2-х полосной автомобильной дороги III категории составляет 7,3км.")
    assert R._scan_text(t2, B["length_route"]) == ["7.3"]
    t3 = ("наличии установленных санитарно-защитных зон таких объектов в пределах участка работ и на "
          "территории размером 1000 м в каждую сторону. Размер СЗЗ предприятия составляет 300 м.")
    assert R._scan_text(t3, B["szz"]) == ["300"]
    # раунд 4: строка ТЭП из ТКР.АД — единица перед числом, метры → км
    assert R._scan_text("Протяженность, м   5920  Расчетная скорость движения,  км/ч  100", B["length_route"]) == ["5.92"]
    assert R._scan_text("Протяженность, м   3827,05  Расчетная скорость", B["length_route"]) == ["3.82705"]
...
def test_unsupported_numbers_location_and_normatives(tmp_path, monkeypatch):
    ...
    assert "18 000 м³" in got and "20 °C" in got and "0,2 м" in got and "3.5.2" not in " ".join(got) and "2024" not in got
    assert V.unsupported_numbers("Объём ГВС 0,2135 м³/с при 450 °C", ctx) == []
    # раунд 4 (тестировщик №4): число с единицей подтверждается только той же парой
    # «число + единица»; даты, ФККО, шифры, нормативы, номера ИЗА — не значения
    ctx2 = "Уровень шума 55 дБА. На стр. 200 приведён расчёт. Ставка 663,2 руб./т. Численность 82 чел."
    assert V.unsupported_numbers("СЗЗ 200 м, шум 55 дБА при норме 70 дБА; ставка 663,2 руб.; 82 человека", ctx2) == ["200 м", "70 дБА"]
    assert V.unsupported_numbers("постановление № 1043 от 31 мая 2023 г., распоряжение № 2409-р от 01.09.2025, "
                                 "ФККО 4 61 010 01 20 5, СанПиН 1.2.3685-21, шифр 717/14/15-П-1, табл. 3.33, ИЗА № 6509", "") == []
    assert "913" in V.normatives_block(scope_text="ставки по ПП РФ № 913") and V.normatives_block(scope_text="про шум") == ""
...
def test_health_ranking_and_model_pick():
    ...
    # порядок по ТЗ 27.07: большие бесплатные → локальные → DeepSeek (платный)
    from pmoos.core.health import TIERS
    big_local = {"ok": True, "tier": TIERS["ollama"], "best_model": "qwen3:32b", "ms": 900}
    small_local = {"ok": True, "tier": TIERS["ollama"], "best_model": "qwen2.5:7b", "ms": 700}
    mis = {"ok": True, "tier": TIERS["mistral"], "best_model": "mistral-large-2512", "ms": 2000}
    dsk = {"ok": True, "tier": TIERS["deepseek"], "best_model": "deepseek-v4-pro", "ms": 1500}
    # бесплатный облачный впереди даже крупной локальной, платный — последний
    order_big = [p for p, _ in rank_working({"ollama": big_local, "mistral": mis, "deepseek": dsk})]
    assert order_big == ["mistral", "ollama", "deepseek"]
    order = [p for p, _ in rank_working({"ollama": small_local, "mistral": mis, "deepseek": dsk})]
    assert order == ["mistral", "ollama", "deepseek"]
    # крупная локальная всё же ближе к бесплатным, чем мелкая
    assert effective_tier("ollama", big_local) < effective_tier("ollama", small_local)
